<div class="z-10">
    ...
    <div x-data="{isShow: true}">
      <div x-show="isShow" class="z-10 absolute top-0 right-0 m-3 w-2/3 md:w-1/3" x-transition:enter="transition transform ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition transform ease-in duration-300" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-1">
        <div class="bg-white border-gray-300 border p-3 flex items-start shadow-lg rounded-md space-x-2">
          ...
          <div class="flex-1 space-y-1">
            <p class="text-base leading-6 font-medium text-gray-700">{{ $message }}</p>
          </div>
    ...
    <div x-data="{isShow: true}">
      <div x-show="isShow" class="z-10 absolute top-0 right-0 m-3 w-2/3 md:w-1/3" x-transition:enter="transition transform ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition transform ease-in duration-300" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-1">
        <div class="bg-white border-gray-300 border p-3 flex items-start shadow-lg rounded-md space-x-2">
          ...
          <div class="flex-1 space-y-1">
            <p class="text-base leading-6 font-medium text-gray-700"> <ul>
