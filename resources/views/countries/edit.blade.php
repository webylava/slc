						...
						<form method="POST" action="{{ route('countries.update',$country->id) }}" enctype="multipart/form-data">
							...
							<div class="md:flex flex-row md:space-x-4 w-full text-xs">
								<div class="mb-3 space-y-2 w-full text-xs">
									...
									<x-input id="name" placeholder="Country Name" class="block mt-1 w-full" type="text" name="name" :value="empty(old('name'))? $country->name:old('name')" required autofocus />
								</div>
								<div class="mb-3 space-y-2 w-full text-xs">
									<x-label for="code" :value="__('Code')" />
									<x-input id="code" placeholder="Country Code" class="block mt-1 w-full" type="text" name="code" :value="empty(old('code'))? $country->code:old('code')" required autofocus />
								</div>
								<div class="mb-3 space-y-2 w-full text-xs">
									<x-label for="iso" :value="__('ISO')" />
									<x-input id="iso" placeholder="Country iso" class="block mt-1 w-full" type="text" name="iso" :value="empty(old('iso'))? $country->iso:old('iso')" required autofocus />
								</div>
							</div>
