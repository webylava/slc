						...
						<form method="POST" action="{{ route('products.update',$product->id) }}" >
							...
							<div class="md:flex flex-row md:space-x-4 w-full text-xs">
								<div class="mb-3 space-y-2 w-full text-xs">
									...
									<x-input id="name" placeholder="product Name" class="block mt-1 w-full" type="text" name="name" :value="empty(old('name'))? $product->name:old('name')" required autofocus />
								</div>
								<div class="mb-3 space-y-2 w-full text-xs">
									<x-label for="price" :asterisk="true" :value="__('Price')" />
									<x-input id="price" placeholder="Price" class="block mt-1 w-full" type="number" name="price" :value="empty(old('price'))? $product->price:old('price')" required autofocus />
								</div>
								<div class="mb-3 space-y-2 w-full text-xs">
									<x-label for="status" :value="__('Product status')" />
									<div class="flex items-center justify-start w-full mb-12">
										<label for="productToggle" class="flex items-center cursor-pointer">
											<!-- toggle -->
											<div class="relative">
												<!-- input -->
												<input type="checkbox" name="status" value="active" id="productToggle" {{ ('active' == $product->status) ? 'checked="checked"' : '' }} class="sr-only">
												<!-- line -->
							...
							<div class="md:flex flex-row md:space-x-4 w-full text-xs">
								<div class="mb-3 space-y-2 w-full text-xs">
									...
									<textarea class="form-textarea mt-1 appearance-none block w-full bg-grey-lighter text-grey-darker border border-gray-300 rounded-lg px-4 block w-full" name="details" id="details" rows="3" placeholder="details">{{ empty(old('details'))? $product->details:old('details') }}</textarea>
								</div>
							</div>
