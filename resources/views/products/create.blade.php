						...
						<form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data">
							...
							<div class="md:flex flex-row md:space-x-4 w-full text-xs">
								<div class="mb-3 space-y-2 w-full text-xs">
									...
									<x-input id="name" placeholder="Product Name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus />
								</div>
								<div class="mb-3 space-y-2 w-full text-xs">
									<x-label for="price" :asterisk="true" :value="__('Price')" />
									<x-input id="price" placeholder="Price" class="block mt-1 w-full" type="number" name="price" :value="old('price')" required autofocus />
								</div>
								<div class="mb-3 space-y-2 w-full text-xs">
									...
									<div class="flex items-center justify-start w-full mb-12">
										<label for="productToggle" class="flex items-center cursor-pointer">
											...
											<div class="relative">
												<!-- input -->
												<input type="checkbox" name="status" value="active" id="productToggle" class="sr-only">
												<!-- line -->
							...
							<div class="md:flex flex-row md:space-x-4 w-full text-xs">
								<div class="mb-3 space-y-2 w-full text-xs">
									...
									<textarea class="form-textarea mt-1 appearance-none block w-full bg-grey-lighter text-grey-darker border border-gray-300 rounded-lg px-4 block w-full" name="details" id="details" rows="3" placeholder="details">{{ old('details') }}</textarea>
								</div>
							</div>
